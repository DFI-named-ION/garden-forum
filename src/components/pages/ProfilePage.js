...
export const ProfilePage = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                if (searchingUser) {
                    let u = await axios.get("https://localhost:44347/api/Users/" + searchingUser.id).then((respnse) => respnse.data);
                    setSearchingUser(u);
                }
    ...
    const handleFieldChange = (fieldName) => (event) => {
        setTempData({ ...tempData, [fieldName]: event.target.value });
    };
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (user) {
                    await axios.put('https://localhost:44347/api/Users/' + user.id, user);
                    setErrors((prevErrors) => ({
                        ...prevErrors,
                        userRequest: '',
                    }));
                ...
                {searchingUser ?
                    <>
                        ...
                        <Grid container spacing={2}>
                            <Grid key={1} item xs={12} sm={4} sx={{textAlign: 'left'}}>
                                <Card>
                                    <CardContent>
                                        <Typography variant="h6">Відображуване ім'я</Typography>
                                        <Typography variant="body1" color="textSecondary">
                                            {isEditing ? (
                                                <TextField
                                                    helperText="Ведіть нове значення"
                                                    variant="standard"
                                                    color="success"
                                                ...
                                                {isEditing ? (
                                                    <>
                                                        ...
                                                        <div style={{ marginLeft: "auto" }}>
                                                            <IconButton onClick={handleCancelClick} color="warning">
                                                                <CancelIcon />
                                                            </IconButton>
                                                        </div>
                                                    </>
